      ...
      {error.show ? (
        ...
        <div className={css.container_content}>
          <div className={css.list_order_status}>
            <div
              className={css.order_status_item}
              style={{
                backgroundColor:
                  request.status === "perlu-diproses" ? "#0091FF" : "white",
              ...
              style={{
                backgroundColor:
                  request.status === "menunggu-konfirmasi"
                    ...
                    : "white",
                color:
                  request.status === "menunggu-konfirmasi" ? "white" : "black",
              }}
              onClick={() => {
                setLoading(true);
            ...
            <div
              ...
              style={{
                backgroundColor:
                  request.status === "pesanan-selesai" ? "#0091FF" : "white",
            ...
            <div
              ...
              style={{
                backgroundColor:
                  request.status === "pesanan-dibatalkan" ? "#0091FF" : "white",
                color:
                  request.status === "pesanan-dibatalkan" ? "white" : "black",
              }}
              onClick={() => {
                setLoading(true);
          ...
          <div className={css.list_orders}>
            {loading ? (
              <div style={{ textAlign: "center" }}>
                <img
                  style={{ width: "80px", paddingTop: "100px" }}
                  src="/assets/image/loading_order.gif"
                  alt="loading_order"
                />
              </div>
            ) : dataOrder.length <= 0 ? (
                  ...
                  <div className={css.box_order} key={item.order_code}>
                    ...
                    >
                      {request.status === "perlu-diproses"
                        ...
                        : request.status === "menunggu-konfirmasi"
                        ? "Menunggu Konfirmasi"
                        : request.status === "pesanan-selesai"
                        ? "Pesanan Selesai"
                        : "Pesanan Dibatalkan"}
                    </div>
                    <div className={css.wrapper_order_data}>
                      ...
                      <div className={css.order_data_and_buyer_2}>
                        <div className={css.order_product_name}>
                          {item.products.name_product}
                        </div>
                        <div className={css.order_product_category}>
                          {item.products.sub_categories.name_sub_category}
                        </div>
                        <div className={css.order_amount}>
                          {item.amount} Akun
                        </div>
                        <div className={css.order_price}>
                          Rp {item.products.price_product}
                        </div>
                      </div>
                      ...
                      {request.status == "perlu-diproses" && (
                        <div className={css.btn_order_action}>
                          <button className={css.btn_send_product}>
                            Kirim Pesanan
                          </button>
                          <button className={css.btn_cancel_product}>
                            Batalkan Pesanan
                          </button>
                        </div>
                      )}
