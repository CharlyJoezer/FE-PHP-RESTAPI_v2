  ...
  function updateAll() {
    ...
    allCheckbox.forEach((item) => {
      if (item.checked) {
        ...
        totalPrice =
          totalPrice + productData.harga_produk * productData.jumlah_produk;
      }
    });
    setTotalPrice(totalPrice);
  }

  let timeoutUpdateAttrDataCart;
  function updateAttrDataCart(value, slug) {
    allCheckbox.forEach((item) => {
      const getData = JSON.parse(item.getAttribute("data"));
      if (getData.slug_produk === slug) {
        clearTimeout(timeoutUpdateAttrDataCart);
        timeoutUpdateAttrDataCart = setTimeout(() => {
              ...
              case 200:
                ...
                allDataCart.forEach((item) => {
                  if (item.checked) {
                    ...
                    totalPrice =
                      totalPrice +
                      productData.harga_produk * productData.jumlah_produk;
                  }
                });
                setTotalPrice(totalPrice);
                break;
            ...
            <div className={css.feature_checklist_all}>
              <input
                ...
                onClick={(e) => {
                  ...
                  dispatch({
                    status: statusEventCheck ? "CHECKALL" : "UNCHECKALL",
                  });
                  updateAll();
                }}
              />
                ...
                return (
                  <div className={css.cart_item} key={item.slug_produk}>
                    <div className={css.checklist_cart_item}>
                      <input
                        ...
                        onClick={(e) => {
                          ...
                          dispatch({
                            status: statusCheck ? "CHECKED" : "UNCHECKED",
                          });
                          updateAll();
                        }}
                      />
                    ...
                    <div className={css.info_product_and_action}>
                      <div className={css.wrapper_data_product}>
                        <div className={css.data_product}>
                          ...
                          <div style={{ paddingLeft: "10px" }}>
                            ...
                            <div className={css.category_product}>
                              {item.kategori_produk}
                            </div>
                            <div className={css.price_product}>
                              Rp {toRupiah(item.harga_produk.toString())}
                            </div>
                          </div>
                        </div>
                      ...
                      <h5 className={css.information_product_text}>
                        Informasi Produk
                      </h5>
          ...
          <div className={css.action_checkout}>
            <div className={css.count_checklist_item}>
              ...
              <div className={css.total_price}>
                Rp{toRupiah(totalPrice.toString())}
              </div>
            </div>
