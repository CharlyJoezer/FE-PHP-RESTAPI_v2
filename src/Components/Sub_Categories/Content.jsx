  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const url =
          BASEURL() +
          `/api/sub_categories/${name_sub_category}/?type=${name_type_category}`;
        const request = await fetch(url, {
          method: "GET",
        });
        if (request.status === 200) {
          ...
          <div className={css.container_header}>
            ...
            <div className={css.input_search}>
              <input
                ...
                name="search"
                placeholder={
                  "Cari Produk " + name_sub_category.replace(/-/g, " ")
                }
                onKeyDown={(e) => {
                  ...
                  if (e.key === "Enter" && search.length > 0) {
                    (async () => {
                      try {
                        ...
                        const url =
                          BASEURL() +
                          `/api/search/${search}?sub_category=${name_sub_category}&type=${name_type_category}`;
                        const request = await fetch(url, {
                          method: "GET",
                        });
                        if (request.status === 200) {
          ...
          <div className={css.container_content}>
            <div className={css.list_type_sub_category}>
              <Link
                ...
                style={{
                  borderBottom:
                    name_type_category.toLowerCase() === "all"
                      ? "2px solid blue"
                      : "none",
                }}
              >
              ...
              {typeCategory.map((i) => {
                return (
                  <Link
                    to={`/category/${category}/${name_sub_category}/${i.name.toLowerCase()}`}
                    key={i.name}
                    className={css.type_sub_category}
                    style={{
                      borderBottom:
                        name_type_category.toLowerCase() ===
                        i.name.toLowerCase()
                          ? "2px solid blue"
                          : "none",
                    }}
            ...
            {productLoad ? (
              ...
              products.length <= 0 && (
                <div className={css.product_not_found}>
                  <img
                    src="/assets/image/search_not_found.jpg"
                    alt="not-found"
                  />
